<template>
    <div>
        ...
        <el-dialog title="添加/修改" :visible.sync="dialogVisible" width="40%">
            <el-form ref="dataForm" :model="sysRegion" label-width="150px" size="small" style="padding-right: 40px;">
                <el-form-item label="地区">
                    <el-input v-model="sysRegion.regionName" />
                </el-form-item>
            </el-form>
...
<script>
import api from '@/api/region/region.js'
export default {
    data() {
        return {
            // 定义table的数据源
    ...
    methods: {
        ...
        batchRemove() {
            ...
            this.$confirm('此操作将永久删除这些地区, 是否继续?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {

                api.deleteBatchRegionByIds(ids).then(response => {
                    // a.给出提示
                    this.$message({
                        type: 'success',
                        message: '删除成功!'
        ...
        removeDataById(id) {
            this.$confirm('此操作将永久删除该地区, 是否继续?', '提示', {
                ...
                api.deleteRegionById(id).then(response => {
                    // a.给出提示
                    this.$message({
                        type: 'success',
                        message: '删除成功!'
        ...
        addRegion() {
            api.addByRegion(this.sysRegion).then(response => {
                // 给出提示框
        ...
        updateRegion() {
            ...
            api.updateByRegion(this.sysRegion).then(response => {
                this.sysRegion={};
                // 给出提示框
