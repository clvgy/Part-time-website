    ...
    <div>
        ...
        <div class="app-container">
            ...
            <el-table v-loading="listLoading" :data="list" stripe border style="width: 100%;margin-top: 10px;"
                ...
                <el-table-column prop="enterpriseId" label="公司名" width="110" :formatter="formatterEnterprise"/>
        ...
        <el-dialog title="添加/修改" :visible.sync="dialogVisible" width="40%">
            <el-form ref="dataForm" :model="sysAddress" label-width="100px" size="small" style="padding-right: 40px;">
                <el-form-item label="公司" prop="enterpriseId">
                    <el-select v-model="sysAddress.enterpriseId" placeholder="请选择公司">
                        <el-option v-for="item in enterpriseList" :key="item.id" :label="item.ename"
                            :value="item.id"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="经度" prop="longitude">
                    <el-input v-model="sysAddress.longitude" />
                </el-form-item>
                <el-form-item label="纬度" prop="latitude">
                    <el-input v-model="sysAddress.latitude" />
                </el-form-item>
            </el-form>
...
import eapi from '@/api/enterprise/enterprise.js'
export default {
    data() {
        return {
    ...
    created() {
        eapi.findAll().then(res=>{
                this.enterpriseList = res.data;
                // alert(this.enterpriseList[0].id)
            })
        this.fetchData();
    ...
    methods: {
        ...
        batchRemove() {
            ...
            this.$confirm('此操作将永久删除这些地址, 是否继续?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {

                api.deleteBatchAddressByIds(ids).then(response => {
                    // a.给出提示
                    this.$message({
                        type: 'success',
                        message: '删除成功!'
        ...
        removeDataById(id) {
            this.$confirm('此操作将永久删除该地址, 是否继续?', '提示', {
                ...
                api.deleteAddressById(id).then(response => {
                    // a.给出提示
                    this.$message({
                        type: 'success',
                        message: '删除成功!'
        ...
        // 更新用户
        updateAddress() {
            api.updateAddress(this.sysAddress).then(response => {
                this.sysAddress = {};
                // 给出提示框
        ...
        // 添加
        addAddress() {
            api.saveAddress(this.sysAddress).then(response => {
                // 提示语
        ...
        // 保存或修改判断
        saveOrUpdate() {
            if (this.sysAddress.id != null && this.sysAddress.id != '') {
                // 修改
                this.updateAddress();
            } else {
                // 添加
        ...
        fetchData(page = 1) {
            this.page = page;
            api.getAddressPageInfo(this.page, this.limit, this.searchObj).then(
                response => {
