        ...
        <div class="app-container">
            ...
            <el-table :data="sysMenuList" style="width: 100%;margin-bottom: 20px;margin-top: 10px;" row-key="id" border
                ...
                <el-table-column label="状态" width="80">
                    <template slot-scope="scope">
                        <el-switch v-model="scope.row.status === 1" :disabled="scope.row.disabled"
...
export default {
    ...
    methods: {
        // 移入修改菜单disable显示状态
        initDisable(row) {
            this.disabled = row.status === 0 ? true : false;
        },
        ...
        switchStatus(row) {
            ...
            if (row.children != null) {
                for (var i = 0; i < row.children.length; i++) {
                    ...
                    // row.children[i].status = status;
                    if (row.children[i].children != null) {
                        for (var j = 0; j < row.children[i].children.length; j++) {
                            if (row.children[i].status === 0) {
                                row.children[i].children[j].disabled = true;
                            } else {
                                row.children[i].children[j].disabled = false;
                            }
                            // row.children[i].children[j].status = status;
                        }
                    }
                }
            }
                ...
                if (response.data[0].status === 0) {
                    ...
                    if (response.data[0].children.length > 0) {
                        for (var i = 0; i < response.data[0].children.length; i++) {
                            if (response.data[0].children[i].status === 0) {
                                response.data[0].children[i].children = {};
                            }
                        }
                    }
                }
